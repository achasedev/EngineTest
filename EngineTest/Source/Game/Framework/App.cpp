...
//-------------------------------------------------------------------------------------------------
static void CreateVkBuffer(VkDeviceSize vkSize, VkBufferUsageFlags vkUsage, VkMemoryPropertyFlags vkPropertyFlags, VkBuffer& vkBuffer, VkDeviceMemory& vkBufferMemory, App* app)
{
...
//-------------------------------------------------------------------------------------------------
static void CreateVkImage(uint32_t width, uint32_t height, VkFormat vkFormat, VkImageTiling vkTiling, VkImageUsageFlags vkUsage, VkMemoryPropertyFlags vkProperties, VkImage& out_vkImage, VkDeviceMemory& out_vkImageMemory)
{
	do this next!
}
...
void App::CreateTexture()
{
	...
	stbi_uc* pixels = stbi_load("Data/Image/debug.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	ASSERT_OR_DIE(pixels != nullptr, "Couldn't load image!");
	...
	CreateVkBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingMemory, this);

	void* data;
	vkMapMemory(m_vkLogicalDevice, stagingMemory, 0, imageSize, 0, &data);
	memcpy(data, pixels, static_cast<size_t>(imageSize));
	vkUnmapMemory(m_vkLogicalDevice, stagingMemory);
	...
	imageInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // Only graphics queue family uses this
	imageInfo.samples = VK_SAMPLE_COUNT_1_BIT; // Multisampling, only relevant for images used as attachments
	imageInfo.flags = 0;

	// Images are just like buffers - they are a way to treat memory, but don't actually allocate any
	VkResult result = vkCreateImage(m_vkLogicalDevice, &imageInfo, nullptr, &m_vkImage);
	ASSERT_OR_DIE(result == VK_SUCCESS, "Couldn't create image!");

	// Now allocate the memory for the image
	VkMemoryRequirements memRequirements;
	vkGetImageMemoryRequirements(m_vkLogicalDevice, m_vkImage, &memRequirements);

	VkMemoryAllocateInfo allocInfo{};
	allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	allocInfo.allocationSize = memRequirements.size;
	allocInfo.memoryTypeIndex = FindMemoryType(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, this);

	result = vkAllocateMemory(m_vkLogicalDevice, &allocInfo, nullptr, &m_vkImageMemory);
	ASSERT_OR_DIE(result == VK_SUCCESS, "Couldn't allocate the image's memory!");

	// Now associate the image with the memory
	vkBindImageMemory(m_vkLogicalDevice, m_vkImage, m_vkImageMemory, 0);
