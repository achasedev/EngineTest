...
BlockLocator BlockLocator::ToEast() const
{
	...
	{
		uint16 newBlockIndex = m_blockIndex & ~Chunk::CHUNK_X_MASK;
...
BlockLocator BlockLocator::ToNorth() const
{
	...
	{
		uint16 newBlockIndex = m_blockIndex & ~Chunk::CHUNK_Z_MASK;
...
BlockLocator BlockLocator::ToAbove() const
{
	...
	// If we're at the top of the chunk, move to the next chunk
	if ((m_blockIndex & Chunk::CHUNK_Y_MASK) == Chunk::CHUNK_Y_MASK)
	{
		uint16 newBlockIndex = m_blockIndex & ~Chunk::CHUNK_Y_MASK;
		return BlockLocator(m_chunk->GetAboveNeighbor(), newBlockIndex);
	}
	else
	{
...
BlockLocator BlockLocator::ToBelow() const
{
	...
	{
		uint16 newBlockIndex = m_blockIndex | Chunk::CHUNK_Y_MASK;
		return BlockLocator(m_chunk->GetBelowNeighbor(), newBlockIndex);
	}
	else
	{
		return BlockLocator(m_chunk, m_blockIndex - Chunk::BLOCKS_PER_Y_LAYER);
